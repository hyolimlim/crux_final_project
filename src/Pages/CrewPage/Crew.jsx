...
const Crew = () => {
  ...
  const getCrew = useCallback(async () => {
    ...
    setLoad(true); //로딩 시작
    await axios
      .get(`https://3.35.22.118/crews/popular?page=${page}&size=6`)
      .then((res) => {
  ...
  const newCrew = useCallback(async () => {
    setLoad(true); //로딩 시작
    await axios
      .get(`http://3.35.22.118/crews?page=${newpage}&size=6`)
      .then((res) => {
        console.log(res.data.data.content);
  ...
  const searchCrew = useCallback(async () => {
    setLoad(true); //로딩 시작
    await axios
      .get(`https://01192mg.shop/crews/search?query=${search}`)
      .then((res) => {
      ...
      <HeaderWrap>
        ...
        <div style={{ width: "120rem", margin: "0 auto", height: "8rem" }}>
          ...
          <img
            src={검색아이콘}
            type="button"
            style={{ width: "3rem", height: "3rem", margin: "0 0 0 -5rem" }}
            onClick={onclickSearchCrew}
          />
        </div>
        <div style={{ width: "120rem", margin: "8rem auto 0 auto" }}>
          <h3>
            <span style={{ fontWeight: "700", margin: "0 2rem 0 0" }}>
              {" "}
              인기 크루{" "}
            </span>
            <span style={{ color: "#cccccc" }} onClick={onclickNewCrew}>
              {" "}
              신규 크루
            </span>
          </h3>
